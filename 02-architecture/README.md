...
Pengguna memulai dengan mengirimkan permintaan melalui dashboard. `Orchestrator Engine` kemudian berkoordinasi dengan `AI Agent` untuk merancang rencana langkah demi langkah.
Setiap langkah dieksekusi melalui `API Gateway` yang melakukan validasi keamanan dan berinteraksi dengan layanan eksternal atau database.
Hasilnya dikumpulkan kembali ke agent untuk refleksi sebelum diberikan jawaban akhir ke pengguna.
